...
string iterate(string word, map<string, Definition> & grammar);
...
int main(int argc, char *argv[])
{
  ...
  //recursion starts with the default "<start>" nonterminal
  string result = iterate("<start>", grammar);
...
string iterate(string word, map<string, Definition> & grammar)
{
  //checks if word given is nonterminal, if it is not, it resturns simply the word
  map<string, Definition>::iterator found = grammar.find(word);
  if(found == grammar.end())return word;
  
  //getting to this point means it is nonterminal and it will expand
  string result = "";

  Production phrase = found->second.getRandomProduction();
  vector<string>::iterator curr;
  for(curr = phrase.begin();curr != phrase.end();curr++) 
    result = result + " " + iterate((*curr), grammar);
